...
* **Database Gate vs. App Gate:** The Supabase publishable `anon` key is public by design. Application proxy (e.g. `src/proxy.ts`) protects pages, but RLS is the **only** gate protecting REST endpoints.
